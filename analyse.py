...
def extract_packet_data(packets):
    ...
    for packet in tqdm(packets, desc="Processing packets", unit="packet"):
        if IP in packet:
            ...
            packet_data.append({"src_ip": src_ip, "dst_ip": dst_ip, "protocol": protocol, "size": size, "timestamp": timestamp})
...
def calculate_rtt(df):
    ...
    for (src_ip, dst_ip), group in df.groupby(["src_ip", "dst_ip"]):
        ...
        rtts = sorted_group["timestamp"].diff().dropna()

        for rtt in rtts:
            if rtt > 0:
                rtt_data.append({"src_ip": src_ip, "dst_ip": dst_ip, "rtt": rtt})

    return pd.DataFrame(rtt_data)

def identify_packet_loss(df):
    loss_data = []
    df['timestamp_diff'] = df.groupby(['src_ip', 'dst_ip'])['timestamp'].diff()
    df = df.dropna(subset=['timestamp_diff'])
    threshold = df['timestamp_diff'].mean() + 3 * df['timestamp_diff'].std()

    packet_loss = df[df['timestamp_diff'] > threshold]

    for _, row in packet_loss.iterrows():
        loss_data.append({"src_ip": row['src_ip'], "dst_ip": row['dst_ip'], "timestamp_diff": row['timestamp_diff']})

    return pd.DataFrame(loss_data)

def print_results(total_bandwidth, protocol_counts_df, ip_communication_table, protocol_frequency, ip_communication_protocols, rtt_df, packet_loss_df):
    ...
    logger.info(tabulate(rtt_df, headers=["Source IP", "Destination IP", "RTT"], tablefmt="grid", floatfmt=".6f"))

    logger.info("\nPacket Loss Instances:\n")
    logger.info(tabulate(packet_loss_df, headers=["Source IP", "Destination IP", "Timestamp Difference"], tablefmt="grid", floatfmt=".6f"))
    
    # Storing the result in .csv files
    protocol_counts_df.to_csv("protocol_counts.csv", index=False)
    ip_communication_table.to_csv("ip_communication_table.csv", index=False)
    ip_communication_protocols.to_csv("ip_communication_protocols.csv", index=False)
    rtt_df.to_csv("rtt.csv", index=False)
    packet_loss_df.to_csv("packet_loss.csv", index=False)

    logger.info("Results saved to CSV files for visualization.")
...
def main(pcap_file, port_scan_threshold):
    ...
    rtt_df = calculate_rtt(df)
    packet_loss_df = identify_packet_loss(df)
    
    print_results(total_bandwidth, protocol_counts, ip_communication_table, protocol_frequency, ip_communication_protocols, rtt_df, packet_loss_df)
